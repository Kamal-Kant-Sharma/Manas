  ...
  return (
    <div className="space-y-6" data-testid="corsi-settings">
      ...
      <section>
        ...
        <Row label="Start length" hint="Number of blocks in the first round">
          <Input type="number" min={2} max={20} value={v.startLength} onChange={(e) => set({ startLength: Number(e.target.value) || 3 })} className="metric" data-testid="corsi-cfg-start" />
        </Row>
        <Row label="Max length" hint="Session ends after reaching this length">
          <Input type="number" min={3} max={20} value={v.maxLength} onChange={(e) => set({ maxLength: Number(e.target.value) || 12 })} className="metric" data-testid="corsi-cfg-max" />
        </Row>
      ...
      <section>
        ...
        <Row label="Recall time limit"><Input type="number" min={2000} max={60000} step={500} value={v.recallTimeoutMs} onChange={(e) => set({ recallTimeoutMs: Number(e.target.value) || 20000 })} className="metric" /></Row>
      </section>
    </div>
  );
}
